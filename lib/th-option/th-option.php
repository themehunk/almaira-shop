...
class almaira_shop_theme_option{
...
function menu_tab() {
    $menu_title = esc_html__('Almaira Shop Options', 'almaira-shop');
    add_theme_page( esc_html__( 'almaira shop', 'almaira-shop' ), $menu_title, 'edit_theme_options', 'thunk_started',array($this,'tab_page'));
...
function admin_scripts( $hook ) {
    ...
    wp_enqueue_script('almaira-shop-admin-load', get_template_directory_uri() . '/lib/th-option/assets/js/th-options.js',array( 'jquery', 'updates' ),'1', true);
    ...
    wp_localize_script( 'almaira-shop-admin-load', 'THAdmin', $data); 
...
function tab_constant(){
    ...
    'welcome'=>sprintf(esc_html__('Welcome To %1s Theme', 'almaira-shop'), esc_html__($theme_data->get( 'Name' ))),
...
$pages = get_pages(array(
    ...
    'meta_value' => 'almaira-shop-frontpage.php'
));
  $post_id = isset($pages[0]->ID)?$pages[0]->ID:false;
if(empty($pages)){
      $post_id = wp_insert_post(array (
       'post_type' => 'page',
       'post_title' => __('Home Page','almaira-shop'),
       'post_content' => '',
       'post_status' => 'publish',
       'comment_status' => 'closed',   // if you prefer
       'ping_status' => 'closed',      // if you prefer
       'page_template' =>'almaira-shop-frontpage.php', //Sets the template for the page.
    ));
  }
      if($post_id){
...
function _check_homepage_setup(){
  ...
  return ($fid == $post_id)?true:false;
	...
	function plugin_install_button($plugin){
            ...
            <h4>'.esc_html( $plugin['plugin_name'] ). ' </h4><a class="plugin-detail thickbox open-plugin-details-modal" href="'.esc_url( $plugin['detail_link'] ).'">'.esc_html__( 'Details & Version', 'almaira-shop' ).'</a>
            </div>';
        ...
        foreach($recommend_plugins[0] as $slug=>$plugin){
            ...
            $button_class = 'install-now button '.$slug;
             ...
             if ( is_plugin_active( $plugin_init ) ) {
                   ...
                   $button_txt = esc_html__( 'Activated', 'almaira-shop' );
...
$boj = new almaira_shop_theme_option();